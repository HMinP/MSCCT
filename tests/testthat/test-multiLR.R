test_that("The given data frame is correctly shaped", {
  ...
  expect_error(multiLR(df[c("time","status","random")]),
...
test_that("The arguments given are correct", {
  ...
  expect_error(multiLR(df), "'status' must be either 0 or 1.")
...
test_that("The output is correct", {
  ...
  for (rho in 0:2){
    testX = multiLR(data_not_PH, test="fh", rho=rho, gamma=0)
